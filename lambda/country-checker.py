...
def lambda_handler(event, context):
    ...
    # obtain source ip from the request context
    clientIp = event["requestContext"]["identity"]["sourceIp"]
    
    print("Source IP: " + clientIp)
    
    # make a get request to the ipstack up lookup api
    resp = requests.get('http://api.ipstack.com/'+clientIp+'?access_key=80c9c50cedef29fb5e52692fb9b8e634')
    
    if resp.status_code != 200:
    # This means something went wrong.
         raise ApiError('GET /tasks/ {}'.format(resp.status_code))
         
    country = resp.json()['country_code']
    
    print("Source IP country: " + country)
    ...
    # can do switching here based on the client country
